...
IrcTuple = collections.namedtuple('IrcTuple', ['index', 'row', 'col']) #設定 tuple 名字 與 設定 tuple 中的元素名稱
XyzTuple = collections.namedtuple('XyzTuple', ['x', 'y', 'z'])

def irc2xyz(coord_irc, origin_xyz, vxSize_xyz, direction_a): #將 IRC 座標轉為 XYZ 座標的函式
    cri_a = np.array(coord_irc)[::-1]  #將 IRC 的座標翻轉為 CRI 以便和 XYZ 對應
    origin_a = np.array(origin_xyz) #原點的偏移量
    vxSize_a = np.array(vxSize_xyz) #將體素大小以 NumPy 陣列表示
    coords_xyz = (direction_a @ (cri_a * vxSize_a)) + origin_a #依照體素大小調整索引，對其與方向矩陣進行矩陣乘法(用 @ 算符)，加上原點的偏移量
    # coords_xyz = (direction_a @ (idx * vxSize_a)) + origin_a
    return XyzTuple(*coords_xyz)

def xyz2irc(coord_xyz, origin_xyz, vxSize_xyz, direction_a): #將 XYZ 座標轉換成 IRC 座標的函式
    origin_a = np.array(origin_xyz)
    vxSize_a = np.array(vxSize_xyz)
    coord_a = np.array(coord_xyz)
    cri_a = ((coord_a - origin_a) @ np.linalg.inv(direction_a)) / vxSize_a #減去原點的偏移量，對方向矩陣的反矩陣進行矩陣乘法，依照體素大小調整索引，與 line 23 順序相反
    cri_a = np.round(cri_a) #在轉換為整數之前先進行適當的進位
    return IrcTuple(int(cri_a[2]), int(cri_a[1]), int(cri_a[0])) #調整順序並轉換成整數
