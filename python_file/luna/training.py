...
class LunaTrainingApp:
    def __init__(self, sys_argv=None):
        ...
        self.time_str = datetime.datetime.now().strftime('%Y-%m-%d_%H.%M.%S') #這裡使用 timestamp 來顯示訓練相關資訊
    ...
    def initOptimizer(self):
        return SGD(self.model.parameters(), lr=0.001, momentum=0.99) #使用 SGD 優化器
    ...
    def computeBatchLoss(self, batch_ndx, batch_tup, batch_size, metrics_g):
        ...
        loss_func = nn.CrossEntropyLoss(reduction='none') # reduction='none' 讓我們可以計算 每個樣本 的損失
        loss_g = loss_func(
            logits_g,
    ...
    def logMetrics(
        ...
        neg_correct = int((negLabel_mask & negPred_mask).sum()) #計算真陰性(預測為陰性且正確)的數量
        pos_correct = int((posLabel_mask & posPred_mask).sum()) #計算真陽性(預測為陽性且正確)的數量
        ...
        metrics_dict['correct/neg'] = neg_correct / np.float32(neg_count) * 100 #計算TN(真陰性率)
        metrics_dict['correct/pos'] = pos_correct / np.float32(pos_count) * 100 #計算TP(真陽性率)
